            ...
            <div class="col-xl-7 mx-auto">
                ...
                <div class="card border-top border-0 border-4 border-primary">
                    <div class="card-body p-5">
                        ...
                        <hr>
                        <form action="{{ route('admin.membership.update', $membership->id) }}" method="POST" class="row g-3">
                            @csrf
                            @method('PUT')
                            <div class="col-12">
                                ...
                                <select class="form-select" id="kategori_membership_id" name="kategori_membership_id" required>
                                    @foreach($kategoriMemberships as $kategori)
                                        <option value="{{ $kategori->id }}" {{ $kategori->id == $membership->kategori_membership_id ? 'selected' : '' }}>{{ $kategori->nama_kategori }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <select class="form-select" id="user_id" name="user_id" required>
                                    @foreach($users->where('role', 'user') as $user)
                                        <option value="{{ $user->id }}" {{ $user->id == $membership->user_id ? 'selected' : '' }}>{{ $user->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <select class="form-select" id="durasi_bulan" name="durasi_bulan" required>
                                    @foreach ($durasiOptions as $key => $value)
                                        <option value="{{ $key }}" {{ $key == $membership->durasi_bulan ? 'selected' : '' }}>{{ $value }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <select class="form-select" id="jenis_pembayaran" name="jenis_pembayaran" required>
                                    ...
                                    <option value="cash" {{ $membership->jenis_pembayaran == 'cash' ? 'selected' : '' }}>Cash</option>
                                    <option value="transfer" {{ $membership->jenis_pembayaran == 'transfer' ? 'selected' : '' }}>Transfer</option>
                                </select>
                            </div>

                            <div class="col-12">
                                <label for="total_bayar" class="form-label">Total Bayar</label>
                                <input type="number" class="form-control" id="total_bayar" name="total_bayar" value="{{ old('total_bayar', $membership->total_bayar) }}" readonly>
                            </div>
