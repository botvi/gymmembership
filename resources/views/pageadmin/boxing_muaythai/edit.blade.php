                ...
                <div class="ps-3">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0 p-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Edit Boxing Muaythai</li>
                        </ol>
                    </nav>
                </div>
            </div>
                    ...
                    <div class="card border-top border-0 border-4 border-primary">
                        <div class="card-body p-5">
                            <div class="card-title d-flex align-items-center">
                                ...
                                <h5 class="mb-0 text-primary">Edit Boxing Muaythai</h5>
                            </div>
                            <hr>
                            <form action="{{ route('admin.boxing_muaythai.update', $existingData->id) }}" method="POST"
                                class="row g-3">
                                @csrf
                                @method('PUT')
                                <select class="form-select text-uppercase" id="boxing_muaythai_id" name="boxing_muaythai_id"
                                    ...
                                    @foreach ($boxingMuaythais as $boxingMuaythai)
                                        <option class="text-uppercase" value="{{ $boxingMuaythai->id }}"
                                            ...
                                            {{ $boxingMuaythai->id == $existingData->boxing_muaythai_id ? 'selected' : '' }}>
                                        ...
                                        @foreach ($users as $user)
                                            @if ($user->role === 'user')
                                                <option value="{{ $user->id }}"
                                                    {{ $user->id == $existingData->user_id ? 'selected' : '' }}>
                                                    {{ $user->nama }}
                                                </option>
                                            @endif
                                        @endforeach
